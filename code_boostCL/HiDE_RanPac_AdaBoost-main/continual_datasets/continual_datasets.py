...
class CUB200(torch.utils.data.Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):        
        ...
        if (not os.path.exists(os.path.join(root, 'CUB_200_2011', 'train'))) or (not os.path.exists(os.path.join(root, 'CUB_200_2011', 'test'))):
            self.split()
    ...
    def split(self):
        ...
        with open(train_test_split, 'r') as f:
            lines = f.readlines()
        for image_path, line in zip(image_paths, lines):
            image_path = image_path.replace('\n', '').split(' ')[-1]
            class_name = image_path.split('/')[0]
...
class Imagenet_R(torch.utils.data.Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):        
        ...
        if not os.path.exists(self.fpath + '/train') and not os.path.exists(self.fpath + '/test'):
            ...
            # print('fafasdfasd whoooooooooooooooooo')
    ...
    def split(self):
        ...
        # print('make dir for train/test')
        
        # print('Train list', len(self.train_file_list))
        for path in self.train_file_list:
            ...
            move(src, dst)
            # print('Move', path, 'done')

        # print('Test list', len(self.test_file_list))
