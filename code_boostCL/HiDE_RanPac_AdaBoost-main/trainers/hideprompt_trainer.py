...
from engines.hide_promtp_wtp_and_tap_engine import train_and_evaluate, evaluate_till_now, train_ranpac
...
def train(args):
    ...
    if args.eval:
        ...
        return
    if args.wols and args.num_view < 1:
        print('Num of views must be >= 1')
    elif args.wols and args.num_view >= 1:
        data_loader, data_loader_per_cls, class_mask, target_task_map = build_continual_dataloader(args)
        acc_matrix = np.zeros((args.num_tasks, args.num_tasks))
        ...
        for task_id in range(args.num_tasks):
            ...
            _ = train_ranpac(model, original_model, data_loader, device,
                                  task_id, class_mask, target_task_map, acc_matrix, args, )
